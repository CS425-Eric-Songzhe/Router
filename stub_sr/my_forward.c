 ...
 *---------------------------------------------------------------------------*/
void
handleForward (struct sr_instance *sr,
 ...
 *---------------------------------------------------------------------------*/
void
forwardPacket (struct sr_instance *sr,
	       uint8_t * packet,
	       unsigned int len, char *interface, uint8_t * desthwaddr)
{
 ...
 *---------------------------------------------------------------------------*/
void
cachePacket (struct sr_instance *sr,
	     uint8_t * packet, unsigned int len, struct sr_rt *rtptr)
{
  struct ip *ipHdr = (struct ip *) (packet + 14);
  int i;
 ...
 *---------------------------------------------------------------------------*/
void
checkCachedPackets (struct sr_instance *sr, int cachedArp)
{
  int i, arpMatch;
  for (i = 0; i < PACKET_CACHE_SIZE; i++)
    {
      if (packetCache[i].len > 0)
		{
		  ...
		  if (packetCache[i].arps <= 5)
			  ...
			  if ((arpMatch = arpSearchCache (packetCache[i].tip)) > -1)
				{
				  ...
				  forwardPacket (sr, (uint8_t *) & packetCache[i].packet,
								 packetCache[i].len,
								 // send it along
								 packetCache[i].nexthop->interface,
								 arpReturnEntryMac (arpMatch));
				  packetCache[i].len = 0;
				}
			  else
				{
			...
			{
			  /* then */
			  icmpSendUnreachable (sr,
								   (uint8_t *) & packetCache[i].packet,
								   packetCache[i].len,
								   packetCache[i].nexthop->interface,
